...
import captionvqa.utils.download as download
import torch
...
from captionvqa.datasets.subset_dataset import captionvqaSubset
from captionvqa.utils.configuration import get_global_config, get_captionvqa_env, get_zoo_config
from captionvqa.utils.general import get_absolute_path
from omegaconf import open_dict


class captionvqaDatasetBuilder(BaseDatasetBuilder):
    ...
    def _download_requirement(
        ...
        download_path = os.path.join(
            get_captionvqa_env("data_dir"), "datasets", dataset_name, dataset_variation
        )
    ...
    def load(self, config, dataset_type, *args, **kwargs):
        ...
        dataset = captionvqaConcatDataset(datasets)
        if split_dataset_from_train:
    ...
    def _split_dataset_from_train(self, dataset, dataset_type):
        if dataset_type in self.config.split_train.keys() or dataset_type == "train":
            ...
            dataset = captionvqaSubset(dataset, indices)
            print(
