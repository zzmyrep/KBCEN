...
from captionvqa.models.transformers.base import (
    BaseTransformer,
...
from captionvqa.modules.encoders import ResNet152ImageEncoder
from captionvqa.utils.build import build_encoder
...
@dataclass
class captionvqaTransformerModalityConfig(BaseTransformerModalityConfig):
    pass


@dataclass
class captionvqaTransformerBackendConfig(BaseTransformerBackendConfig):
    pass


# Can be used with captionvqat or captionvqa_transformer
@registry.register_model("captionvqat")
@registry.register_model("captionvqa_transformer")
class captionvqaTransformer(BaseTransformer):
    @dataclass
    class Config(BaseTransformer.Config):
        model: str = "captionvqat"
        ...
        backend: BaseTransformerBackendConfig = captionvqaTransformerBackendConfig(
            type="huggingface"
        )
        modalities: List[BaseTransformerModalityConfig] = field(
            default_factory=lambda: [
                captionvqaTransformerModalityConfig(
                    type="text",
                    key="text",
                    position_dim=512,
                    embedding_dim=768,
    ...
    # Backward compatibility for code for old captionvqat checkpoints
    @classmethod
    ...
    def config_path(cls) -> str:
        return "configs/models/captionvqa_transformer/defaults.yaml"
    ...
    def _find_unique_encoder_key(self, key):
        assert key in self.encoders, f"captionvqaT doesn't have {key} encoder."
        for modality in self.config.modalities:
            if modality.key == key:
                assert (
                    len([m for m in self.config.modalities if m.key == modality.key])
                    == 1
                ), f"captionvqaT has multiple modalities with the same key {key}."
                assert (
