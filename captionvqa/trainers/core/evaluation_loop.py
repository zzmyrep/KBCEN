...
from captionvqa.common.sample import to_device
from captionvqa.utils.distributed import gather_tensor, is_main, is_xla
...
class TrainerEvaluationLoopMixin(ABC):
    ...
    def _can_use_tqdm(self, dataloader: torch.utils.data.DataLoader):
        ...
        1) should have `__len__` property defined 检查 tqdm 是否可以与数据加载器一起正常使用
        2) calling len(x) should not throw errors. 调用 len(x) 不应引发错误
        """
