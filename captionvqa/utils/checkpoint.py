...
from captionvqa.utils.checkpoint_updater import get_pretrained_state_mapping_checkpoint
from captionvqa.utils.configuration import get_captionvqa_env, load_yaml
from captionvqa.utils.distributed import is_main, is_xla, open_if_main, synchronize
from captionvqa.utils.download import download_pretrained_model
from captionvqa.utils.file_io import PathManager
from captionvqa.utils.general import get_current_device, updir
from captionvqa.utils.xla import save_xla_ckpt
...
def _hack_imports():
    ...
    sys.modules["pythia"] = importlib.import_module("captionvqa")
    sys.modules["pythia.utils.configuration"] = importlib.import_module(
        "captionvqa.utils.configuration"
    )
...
def get_ckpt_path_from_folder(folder) -> str:
    ...
    assert (
        len(ckpts) == 1
    ), "None or multiple checkpoints files. captionvqa doesn't know what to do."
...
def get_config_from_folder_or_ckpt(
    ...
    if len(configs) > 0:
        assert len(configs) <= 1, (
            ...
            + "captionvqa doesn't know what to do."
        )
...
class Checkpoint:
    def __init__(self, trainer):
        ...
        self.save_dir = get_captionvqa_env(key="save_dir")
