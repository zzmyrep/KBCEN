...
def _should_skip_if_mismatch(
    ...
    # In case of either mismatch or match both, captionvqa will try
...
class captionvqaToPLCheckpointUpdater:
    def __init__(self):
        pass
    ...
    def update_checkpoint(
        ...
        If the checkpoint is a model only checkpoint:
            1. If it is an captionvqa checkpoint, convert to lightning format
        ...
        If the checkpoint is a trainer only checkpoint:
            ...
            2. do the checkpoint trainer state update from captionvqa to lightning
        ...
        if not is_pl_trainer_checkpoint(checkpoint):
            self._update_trainer_checkpoint_from_captionvqa(checkpoint=checkpoint, model=model)

    def _update_trainer_checkpoint_from_captionvqa(
        self, checkpoint: Dict[str, Any], model: Any
    ) -> None:
        """updates checkpoint from the captionvqa format to lightning format.
        captionvqa checkpoint is with keys:
        ...
        if "lr_scheduler" in checkpoint:
            ...
            # lr_schedulers to be present to resume checkpoint while in captionvqa, it is
    ...
    def _update_model_checkpoint(
        ...
        if not is_pl_model_checkpoint(checkpoint):
            self._update_model_checkpoint_from_captionvqa(checkpoint)
    ...
    def _update_model_checkpoint_from_captionvqa(self, checkpoint: Dict[str, Any]) -> None:
        tmp_checkpoint = dict(checkpoint)
        checkpoint.clear()
