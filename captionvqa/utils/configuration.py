...
from captionvqa.utils.env import import_user_module
from captionvqa.utils.file_io import PathManager
from captionvqa.utils.general import get_absolute_path, get_captionvqa_root
...
def load_yaml(f):
    ...
    try:
        ...
        relative = os.path.abspath(os.path.join(get_captionvqa_root(), f))
    ...
    captionvqa_root_dir = get_captionvqa_root()

    for include in includes:
        original_include_path = include
        include = os.path.join(captionvqa_root_dir, include)

        # If path doesn't exist relative to captionvqa root, try relative to current file
...
def get_captionvqa_cache_dir():
    config = get_global_config()
    cache_dir = config.env.cache_dir
    # If cache_dir path exists do not join to captionvqa root
    if not os.path.exists(cache_dir):
        cache_dir = os.path.join(get_captionvqa_root(), cache_dir)
    return cache_dir


def get_captionvqa_env(key=None):
    config = get_global_config()
    if key:
...
def resolve_cache_dir(env_variable="captionvqa_CACHE_DIR", default="captionvqa"):
    ...
    if not PathManager.exists(cache_path):
        try:
            ...
            cache_path = os.path.join(get_captionvqa_root(), ".captionvqa_cache")
            PathManager.mkdirs(cache_path)
...
class Configuration:
    ...
    def import_user_dir(self):
        # Try user_dir options in order of captionvqa configuration hierarchy
