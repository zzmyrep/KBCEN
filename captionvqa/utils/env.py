...
def import_user_module(user_dir: str):
    ...
    from captionvqa.utils.general import get_absolute_path  # noqa

    logger = logging.getLogger(__name__)
    if user_dir:
        if registry.get("__captionvqa_user_dir_imported__", no_warning=True):
        ...
        sys.modules["captionvqa_user_dir"] = sys.modules[module_name]
        ...
        registry.register("__captionvqa_user_dir_imported__", True)
...
def import_files(file_path: str, module_name: str = None):
    ...
    ```
    from captionvqa.utils.env import import_files
...
def setup_imports():
    ...
    root_folder = registry.get("captionvqa_root", no_warning=True)

    if root_folder is None:
        root_folder = os.path.dirname(os.path.abspath(__file__))
        root_folder = os.path.join(root_folder, "..")

        environment_captionvqa_path = os.environ.get("captionvqa_PATH", os.environ.get("PYTHIA_PATH"))

        if environment_captionvqa_path is not None:
            root_folder = environment_captionvqa_path

        registry.register("pythia_path", root_folder)
        registry.register("captionvqa_path", root_folder)
    ...
    importlib.import_module("captionvqa.common.meter")
    ...
    for f in files:
        ...
        if f.endswith(".py") and not f.endswith("__init__.py"):
            ...
            module = ".".join(["captionvqa"] + splits[import_prefix_index:-1] + [module_name])
...
def teardown_imports():
    ...
    registry.unregister("captionvqa_path")
