...
import torch
from captionvqa.utils.distributed import get_rank, get_world_size, is_xla
...
def get_captionvqa_root():
    from captionvqa.common.registry import registry

    captionvqa_root = registry.get("captionvqa_root", no_warning=True)
    if captionvqa_root is None:
        captionvqa_root = os.path.dirname(os.path.abspath(__file__))
        captionvqa_root = os.path.abspath(os.path.join(captionvqa_root, ".."))
        registry.register("captionvqa_root", captionvqa_root)
    return captionvqa_root
...
def get_absolute_path(paths):
    ...
    if isinstance(paths, str):
        ...
        captionvqa_root = get_captionvqa_root()
        user_dir = get_captionvqa_env(key="user_dir")
        if user_dir:
            possible_paths.append(os.path.join(user_dir, paths))
            # check in relative to captionvqa relative user dir
            possible_paths.append(os.path.join(captionvqa_root, "..", user_dir, paths))

        # Relative to root folder of captionvqa install
        possible_paths.append(os.path.join(captionvqa_root, "..", paths))
        # Relative to captionvqa root
        possible_paths.append(os.path.join(captionvqa_root, paths))
...
def extract_loss(report: Dict[str, Any], loss_divisor: int) -> torch.Tensor:
    ...
    # Since losses are batch averaged in captionvqa, this makes sure the
    # scaling is right.
