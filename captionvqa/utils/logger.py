...
from captionvqa.utils.distributed import get_rank, is_main, is_xla
from captionvqa.utils.file_io import PathManager
from captionvqa.utils.timer import Timer
...
def setup_output_folder(folder_only: bool = False):
    ...
    """
    save_dir = get_captionvqa_env(key="save_dir")
    ...
    env_log_dir = get_captionvqa_env(key="log_dir")
    if env_log_dir:
...
def setup_logger(
    ...
    **kwargs,
    ...
    """
    Initialize the captionvqa logger and set its verbosity level to "INFO".
    ...
    Args:
        ...
        name (str): the root module name of this logger. Defaults to "captionvqa".
    ...
    # Remove existing handlers to add captionvqa specific handlers
    if clear_handlers:
...
def setup_very_basic_config(color=True):
    ...
    # log a message even before logging is setup by captionvqa.
...
def log_class_usage(component_type, klass):
    """This function is used to log the usage of different captionvqa components."""
    identifier = "captionvqa"
...
# ColorfulFormatter is adopted from Detectron2 and adapted for captionvqa
