...
from captionvqa.utils.text import VocabDict
...
class GraphNetworkBare(BaseModel):
    ...
    # Each model in captionvqa gets a dict called sample_list which contains
    ...
    def forward(self, sample_list):
        ...
        # For loss calculations (automatically done by captionvqa
        ...
        # captionvqa will automatically calculate loss
        return output


# Do indirect path stuff with captionvqa
def captionvqa_indirect(path):
    if os.path.exists(path):
        return path
    else:
        path = os.path.join(os.getenv("captionvqa_DATA_DIR"), "datasets", path)
        return path


# Graph network module
# Can be added as part of a larger network, or used alone using GraphNetworkBare bare最基本的
class GraphNetworkModule(nn.Module):
    ...
    def __init__(self, config, config_extra=None):
        ...
        raw_graph = torch.load(captionvqa_indirect(config.kg_path))
        ...
        torch.save(self.graph_answers, captionvqa_indirect(config.graph_vocab_file))

        # If features have w2v, initialize it here
        node2vec_filename = captionvqa_indirect(config.node2vec_filename)
        ...
        if not valid_node2vec:
            node2vec, node_names_dbg, no_match_nodes = prepare_embeddings(
                node_names,
                captionvqa_indirect(config.embedding_file),
        ...
        # Otherwise, we feed into every graph node at start
            ...
            if (
                ...
                # print('self.config_extra["compress_crossmodel"]',type(self.config_extra["compress_crossmodel"]), self.config_extra["compress_crossmodel"])
                assert not self.config_extra["compress_crossmodel"]
    ...
    def get_dataset_info(self, config):
        # Load dataset info
        dataset_data = torch.load(captionvqa_indirect(config.dataset_info_path))
    ...
    def get_answer_info(self, config):
        ...
        # Recreates captionvqa answer_vocab here essentially
        # 在此处基本上重新创建captionvqa answer_voab
        answer_vocab = VocabDict(captionvqa_indirect(config.vocab_file))
        ...
        if config.okvqa_v_mode in ["v1.0", "v1.0-121", "v1.0-121-mc"]:
            ...
            tx_data = torch.load(captionvqa_indirect(config.ans_translation_file))
            if config.okvqa_v_mode in ["v1.0-121", "v1.0-121-mc"]:
                old_graph_vocab = torch.load(captionvqa_indirect(config.old_graph_vocab_file))
